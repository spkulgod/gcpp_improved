...
def val_model(pra_model, pra_data_loader,train_it):
    ...
    # train model using training data
    trajectories = {"predicted":[],"probs":[],"index":[]}
    for iteration, (ori_data, A, _, img) in enumerate(pra_data_loader):
        ...
        for now_history_frames in range(6, 7):
            ...
            all_tokens.extend(data_token_ids.squeeze().detach().cpu().numpy()) # Shape: (N,50) ---> Use this for json file - id index starts from 1 
...
#     my_print('P2'.format(P2))
    ...
    trajectories["index"] = np.array(all_tokens).tolist()
    
    save_json(trajectories,train_it)
